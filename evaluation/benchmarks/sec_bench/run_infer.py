...
def get_instruction(instance: pd.Series, metadata: EvalMetadata):
    ...
    if task_type == 'poc':
        ...
        instruction = (
            ...
            'You can invoke tools via `bash` (to run `rg`, `grep`, `find`, `cat`, `sed`, `python`, `gdb -x`, etc.), `ipython` for Python snippets, `str_replace_editor`/`llm_based_edit` for file edits, and `think` for private scratch space. Avoid `browser` unless the task explicitly requires it.\n'
